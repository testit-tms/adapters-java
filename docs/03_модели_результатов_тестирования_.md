...
3.  **Удобство передачи:** Собранные и упакованные в эти модели данные легко передавать между компонентами адаптера (например, от [Слушателя событий фреймворков](04_слушатели_событий_фреймворков_.md) к [Клиенту API](07_клиент_api.md)) и отправлять на сервер Test IT.
...
5.  **Передача Менеджеру:** На каждом этапе (старт/стоп теста, старт/стоп шага) информация передается [Менеджеру Адаптера](06_менеджеру_адаптера_.md), который решает, что с ней делать дальше (например, отправить в Test IT через [Клиент API](07_клиент_api.md)).
