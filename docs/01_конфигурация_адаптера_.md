...
Когда адаптеру нужно отправить данные в Test IT (например, результаты теста), он обратится к объекту `ClientConfiguration`, чтобы получить URL, токен и другие нужные параметры для [Клиента API](07_клиент_api.md).
