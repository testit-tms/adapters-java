...
Эта аннотация позволяет **связать автотест с рабочими элементами (Work Items)** в Test IT. Указывается **Глобальный ID (GID)** рабочего элемента из Test IT. Можно указать несколько ID.
...
public class FeatureTests {
    ...
    @Test
    @ExternalId("Feat-123")
    @WorkItemIds({"123", "223"}) // Связываем с 123 и 223
...
> ⚠️ **Важно**: Начиная с версии 2.2.0, если значение из аннотации `@WorkItemIds` не найдено в TMS, то результат теста **НЕ будет загружен**.
...
6.  **Отправка результатов:** После завершения теста (успешного или неуспешного) слушатель передает собранную модель результата [Менеджеру Адаптера](06_менеджеру_адаптера_.md), который, в свою очередь, использует [Клиент API](07_клиент_api.md) для отправки этих обогащенных данных на сервер Test IT.
