...
### BulkAutotestHelper: Оптимизация массовой загрузки тестов

Для оптимизации процесса загрузки большого количества тестов в Test IT, в адаптере используется вспомогательный класс `BulkAutotestHelper`. Этот класс позволяет группировать операции создания и обновления автотестов, а также отправку их результатов, что значительно повышает производительность при массовой загрузке.

Основные функции `BulkAutotestHelper`:
- Группировка автотестов для пакетного создания (`addForCreate`)
- Группировка автотестов для пакетного обновления (`addForUpdate`)
- Автоматическая отправка накопленных данных при достижении определенного порога (100 тестов)
- Финализация загрузки всех накопленных данных (`teardown`)

Этот класс особенно полезен при выполнении больших тестовых наборов, где количество тестов может достигать тысяч.
